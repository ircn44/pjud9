  ...
  <v-container>
    <v-row>
      ...
      <v-col sm="6" md="6" lg="6">
        ...
        <!-- Espacio para el texto de nomenclaturas -->
        <div v-if="nomenclatureText" class="nomenclature-text">
          {{ nomenclatureText }}
        </div>
      </v-col>
      ...
      <v-col sm="6" md="6" lg="6">
        ...
        <div ref="oficioText">
          ...
          <p class="intro-text">Resolviendo presentación de fecha XXXX de XXXX de XXXX, a folio XXX:</p>
...
export default {
  ...
  computed: {
    nomenclatureText() {
      ...
      // Agregar nomenclaturas según los checkboxes seleccionados
      if (this.instituciones.includes("Servel") || this.instituciones.includes("PDI")) {
        nomenclatures.push("851-Ofíciese");
      }
      if (this.instituciones.includes("SII")) {
        nomenclatures.push("1390- Diligencias SII");
      }
      if (this.instituciones.includes("Tesorería")) {
        nomenclatures.push("1391- Solicita información domicilio TGR");
      }

      // Construir el mensaje basado en las nomenclaturas
      if (nomenclatures.length === 1) {
        return `Debes usar la nomenclatura '${nomenclatures[0]}'`;
      } else if (nomenclatures.length > 1) {
        const last = nomenclatures.pop();
        return `Debes usar las nomenclaturas '${nomenclatures.join(", ")} y ${last}'`;
      }
      return ''; // No muestra texto si no hay checkboxes relevantes seleccionados
    }
  },
...
.nomenclature-text {
  ...
  margin-top: 20px; /* Espacio más grande entre botones y texto */
}
